...
class Board
  ...
  def two_computer_markers_in_line
    WINNING_LINES.each do |line|
      squares = @squares.values_at(*line)
      markers = squares.select {|element| element.marker == TTTGame::COMPUTER_MARKER}.collect(&:marker)
      if markers.size == 2
  ...
  def two_human_markers_in_line
    WINNING_LINES.each do |line|
      squares = @squares.values_at(*line)
      markers = squares.select {|element| element.marker == TTTGame::HUMAN_MARKER}.collect(&:marker)
      if markers.size == 2
        empty_square = squares.select {|element| element.marker == Square::INITIAL_MARKER}[0]
        return @squares.key(empty_square)
      else
...
class TTTGame
  ...
  def display_scores # probably a private method like the rest?
